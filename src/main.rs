    ...
    Command::new("pqencrypt")
        ...
        .subcommand(
            Command::new("encrypt")
                ...
                .arg(arg!(--pass <PASS> "Passphrase to derive encryption key").required(true))
        ...
        .subcommand(
            Command::new("decrypt")
                ...
                .arg(arg!(--pass <PASS> "Passphrase to derive decryption key").required(true))
...
async fn check() {
    ...
    if let Some(sub_matches) = matches.subcommand_matches("encrypt") {
        let passphrase = sub_matches.get_one::<String>("pass").expect("Passphrase is required");
    ...
    if let Some(sub_matches) = matches.subcommand_matches("decrypt") {
        ...
        let decrypt_result = if let Some(file_path) = sub_matches.get_one::<String>("file") {
            ...
            if message.find("-----BEGIN ENCRYPTED MESSAGE-----").is_some() {
                let message = Decrypt::extract_encrypted_message(&message);
                Decrypt::decrypt(secret_key_path, ciphertext_path, None, Some(&message.unwrap().as_slice()), hmac_key).await
            } else {
